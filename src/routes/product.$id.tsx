...
export const Route = createFileRoute("/product/$id")({
  loader: async ({ params }) => {
    ...
    const product = catalog.products.find((item) => item.id === params.id);
      ...
      <div className="mt-8 grid md:grid-cols-2 gap-10 lg:gap-16">
        <div className="space-y-3">
          <div className="aspect-[4/5] bg-secondary overflow-hidden">
            <img src={product.image} alt={product.name} width={800} height={1000} decoding="async" fetchPriority="high" className="size-full object-cover" />
          </div>
          <div className="grid grid-cols-4 gap-3">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="aspect-square bg-secondary overflow-hidden">
                <img src={product.image} alt="" loading="lazy" decoding="async" className="size-full object-cover opacity-90 hover:opacity-100" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <p className="text-[11px] uppercase tracking-[0.28em] text-primary">{categoryName(product.category, catalog.categories)}</p>
